...
- Built **entwurf**: a garden-citizen dispatch substrate — six agent harnesses address one another by id without owning each other's transcript, auth, or runtime. Claude Code, GitHub Copilot CLI and OMP ship as mailbox-backed self-fetch citizens, Antigravity as a native-push citizen with a managed install surface, pi through a control-socket adapter; Codex has a verified delivery probe but no managed lane yet. Published on npm ([v0.17.2](https://github.com/junghan0611/entwurf/releases/tag/v0.17.2)).
- Built **agent-config**: the skills SSOT and proving ground for the above — 40+ agent skills over notes, agenda, bibliography, health and git history, hardened against a real daily workload before `entwurf` absorbs a surface. It holds no memory of its own; it is the interface through which memory is called
...
- Built and published a digital garden of 2,200+ pages ([notes.junghanacs.com](https://notes.junghanacs.com)), backed by 3,500+ org-mode notes and 1,600+ consecutive days of journaling
- Built a second **Clojure/SCI kernel runtime** for [prime-agent](https://github.com/junghan0611/prime-agent) (a fork of PrimeIntellect-ai's RLM harness), compiled with GraalVM native-image and now its default kernel — an experiment measuring where a Lisp workspace carries the persistent-REPL agent loop and where it stops. The Python arm is kept as the oracle so it stays a comparison; no performance claim until the Python contracts are answered test-for-test
- Gave an agent its **own public address** — [aionsclubs.org](https://aionsclubs.org), where one of the bots writes and publishes from inside its own container. I operate the infrastructure; the editorial authority is deliberately not mine
...
- Built **memex-kb**, a document toolchain that makes Korean content machine-legible: `hwpx2org` for the Korean word processor format no toolchain wants to touch, `scanpdf2org` with vision transcription for scanned paper, `epub2org` / `html2epub`, `org2odtdoc` for the round trip back into office formats, `textlint-ko` for Korean prose linting, and a proposal pipeline. Org-mode is the meta-document everything passes through. **Any Korean organization working with HWP files or scanned administrative documents hits this wall; I have a toolchain for it.**
...
| [agent-config](https://github.com/junghan0611/agent-config) | Skills SSOT and proving ground — 40+ agent skills that call memory rather than hold it | TypeScript |
...
| [prime-agent](https://github.com/junghan0611/prime-agent) *(fork)* | Clojure/SCI kernel runtime for an RLM agent harness — second arm beside the CPython oracle, GraalVM native-image | Clojure · TypeScript |
| [sorge](https://github.com/junghan0611/sorge) | Cross-repository care — one ledger of human judgements, nothing derivable stored | Markdown |
| [apply](https://github.com/junghan0611/apply) | Evidence-first application operations; history rewritten for publication rather than squashed | Python · Markdown |
