...
읽습니다. 논증은 한 줄입니다 — /에이전트를 수십 개 붙여도 지나간 시간은 만들어낼 수 없습니다./

원본(FULL)은 제목과 참조를 품으므로 *공개하지 않습니다.* 이 문서에는 allowlist로 집계한 공개
projection만 연결하고, 스냅샷·코드·내용 해시와 소스 상태를 함께 제시합니다.
...
#+begin_export html
<div class="evidence-mount" data-component="timeline-projection" data-placeholder="true">
  <p><strong>Timeline projection mount.</strong> 공개 allowlist 산출물이 생성되면 이 자리에
  2026 coverage와 깊이별 판독을 연결한다. LOCAL <code>axis.html</code>은 임베드하지 않는다.</p>
</div>
#+end_export
